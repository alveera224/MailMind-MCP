...
def save_draft(
    ...
    """Build a reply and save it as a draft in the Drafts folder."""
    client = get_client_from_context(ctx)
    # Fetch the original email
    original_email = client.fetch_email(original_email_id)
    if not original_email:
        return f"Original email with UID {original_email_id} not found."
    # Parse addresses
    to_addr = [EmailAddress.parse(addr.strip()) for addr in to.split(",") if addr.strip()]
    from_addr = EmailAddress.parse(from_)
    cc_addr = [EmailAddress.parse(addr.strip()) for addr in cc.split(",") if cc and addr.strip()]
    ...
    if draft_uid:
        return f"Draft saved successfully with UID {draft_uid} in Drafts folder."
    else:
